...
type DamageScenarioItem = {
  id: number | string
  name: string
  threat_scenario_id?: number | string
...
type AttackStepItem = {
  ...
  name: string
  component_id?: number | string
  threat_scenario?: Array<number | string>
...
type ThreatScenarioItem = {
  id: number | string
  name: string
  attack_step?: Array<number | string>
  damage_scenario?: Array<number | string>
  compromises?: Array<{
    component_id?: number | string
    compromised_part_cia?: string
  }>
...
function makeId(prefix: string, value: string | number) {
  return `${prefix}-${value}`
}
...
function leafFromIdRef(label: string, id: string | number): TreeNode {
  return {
    id: `${label.toLowerCase().replace(/\s+/g, '-')}-${id}`,
    label: `${label} ${id}`,
    kind: 'leaf',
    badge: 'id-ref',
    raw: { id },
...
function buildThreatScenarioNode(
  ...
  if (visited.has(visitKey)) {
    return {
      id: makeId('threat-scenario', String(threatScenario.id)),
  ...
  const linkedAttackSteps = asArray<number | string>(threatScenario.attack_step)
  ...
  const linkedDamageScenarios = asArray<number | string>(
    threatScenario.damage_scenario
  )
  ...
  if (
    ...
    children.push({
      ...
      children: threatScenario.compromises.map((c, index) => ({
        ...
        label: `Component ${c.component_id ?? '?'} → ${c.compromised_part_cia ?? 'CIA'}`,
        kind: 'leaf',
        raw: c,
      })),
    })
...
function buildDamageScenarioNode(
  ...
  if (damageScenario.threat_scenario_id != null) {
    const ts = threatScenariosById.get(
      String(damageScenario.threat_scenario_id)
    )
    if (ts) {
      children.push(
      ...
      children.push(
        leafFromIdRef('Threat Scenario', damageScenario.threat_scenario_id)
      )
    }
  }

  return {
    id: makeId('damage-scenario', String(damageScenario.id)),
...
function buildAttackStepNode(
  ...
  if (visited.has(visitKey)) {
    return {
      id: makeId('attack-step', String(attackStep.id)),
  ...
  const tsIds = asArray<number | string>(attackStep.threat_scenario)
  if (tsIds.length) {
    children.push({
...
function buildControlNode(control: ControlItem): TreeNode {
  ...
  return {
    id: makeId('control', String(control.id)),
...
function buildComponentNode(
  ...
  if (dataEntities.length) {
    children.push({
      ...
      children: dataEntities.map((entity) => ({
        id: makeId('data-entity', String(entity.id)),
  ...
  if (technologies.length) {
    children.push({
      ...
      children: technologies.map((tech) => ({
        id: makeId('technology', String(tech.id)),
  ...
  return {
    id: makeId('component', String(detail.id)),
...
function extractAttackSteps(data: unknown): AttackStepItem[] {
  return extractList<AttackStepItem>(data, 'attack_step')
}

function extractThreatScenarios(data: unknown): ThreatScenarioItem[] {
  return extractList<ThreatScenarioItem>(data, 'threat_scenario')
}

function extractDamageScenarios(data: unknown): DamageScenarioItem[] {
  return extractList<DamageScenarioItem>(data, 'damage_scenario')
}
...
export async function getProjectTree(
  ...
  const componentsResponse = await safeGet<unknown>(
    '/component',
    projectId,
    true
  )
